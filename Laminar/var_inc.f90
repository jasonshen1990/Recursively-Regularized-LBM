...
!=============================================================================
      ...
      integer,parameter:: nx7 = 61,nx = nx7-1, ny = 20, nz = 2
...
!     Building
      integer,parameter :: nbd = 1 ! number of buildings
      integer,dimension(nbd) :: bheight, blenh, bwidh
      integer,dimension(nbd) :: bcenty, bcentz
      integer nypd,nzpd
      real fbsf, fbhr

      !Diagnostic and data output paramters
      integer,parameter:: ndiag = 50, nstat = 1000  , nspec=500
      integer,parameter:: nflowout = 50000, npartout = 5000, ntime = 1000
      ...
      real,parameter:: rho0 = 1.0, rhopart = 10.0
      integer,parameter:: npart = 6022
      real,parameter:: radf = 4.0, mingap = 3.0, mingap_w =3.0
      integer,parameter:: irelease = 1800010
      integer,parameter:: iprocrate = 2
      real,parameter:: et0 = 2.354998E+01 

!     LES variables
      logical iLES, iTRACER, newTRACER
      real,allocatable,dimension(:,:,:) :: tau_turb
      real, parameter :: cw = 0.5, cv = 0.036
      ...
      real force_in_y,ustar,Rstar,ystar,force_mag,rad
      real pi, pi2, anu, visc, tau, u0in, viscb 
      real vscale, escale, dscale, tscale 
      real omegepsl, omegepslj, omegxx
      real alpha17, beta17, beta18, beta19, gamma20, gamma21
      real gamma22, alpha26, beta26, zeta23, zeta24, zeta25
      real s1, s2, s3, s4, s5, s7, s9, s10, s13, s16, s17, s18, s20,  &
              s23, s26 !Relaxation terms
      real coef1, coef2, coef3, coef4, coef5, coef3i, coef4i
      real val1, val2, val4, val5, val6, val7, val8, val11,       &
           val1i, val2i, val4i, val5i, val6i, val7i, val8i, val11i
      ...
      real q9,q12,q18,q24,q27,q54,q72,q108
      integer,dimension(0:npop-1):: cix, ciy, ciz, ipopp, ipref
      ...
      integer s
      real,allocatable,dimension(:,:,:,:):: f,f_post_coll
      integer,dimension(13):: ipswap
      integer,dimension(14):: ipstay
      ...
      integer,allocatable,dimension(:,:,:):: ifill,ifspecial
      integer,allocatable,dimension(:,:,:):: icover
      integer,allocatable,dimension(:,:,:,:):: particleindex
      real,allocatable,dimension(:,:,:,:):: linkindex
      ...
      real,allocatable,dimension(:,:,:):: hydrofx,hydrofy,hydrofz
      real,allocatable,dimension(:,:,:):: hydrofx2,hydrofy2,hydrofz2
      real,allocatable,dimension(:,:,:):: pworkx,pworky,pworkz
