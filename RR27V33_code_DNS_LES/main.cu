...
int main() {
    ...
    for (int istep = continue_step + 1; istep <= NEND; istep++) {
        ...
        if (USE_LES) {
        LES_WALE<<<grid, block>>>(d_ux, d_uy, d_uz, d_taut);    // LES WALE model
        CHECK_CUDA_ERROR(cudaGetLastError());
        CHECK_CUDA_ERROR(cudaDeviceSynchronize());
        }

        collision_RR<<<grid, block>>>(d_f, d_force_realx, d_force_realy, d_force_realz, 
                                       d_rho, d_ux, d_uy, d_uz, d_taut);   // collision
        CHECK_CUDA_ERROR(cudaGetLastError());
        CHECK_CUDA_ERROR(cudaDeviceSynchronize());

        streaming<<<grid, block>>>(d_f, d_f_temp);   // Streaming
        CHECK_CUDA_ERROR(cudaDeviceSynchronize());
        BB<<<grid, block>>>(d_f, d_f_temp); 
        ...
        RRBC<<<grid, block>>>(d_f, d_force_realx, d_force_realy, d_force_realz,    //boundary treatment
                              d_rho, d_ux, d_uy, d_uz, d_taut);
        // FDBC<<<grid, block>>>(d_f, d_force_realx, d_force_realy, d_force_realz,    //boundary treatment
        //                       d_rho, d_ux, d_uy, d_uz, d_taut);
        CHECK_CUDA_ERROR(cudaGetLastError());
        CHECK_CUDA_ERROR(cudaDeviceSynchronize());

        macrovar_BN<<<grid, block>>>(d_f, d_force_realx, d_force_realy, d_force_realz, 
                                 d_rho, d_ux, d_uy, d_uz);
        CHECK_CUDA_ERROR(cudaGetLastError());
        CHECK_CUDA_ERROR(cudaDeviceSynchronize());
